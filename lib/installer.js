...
function writeConfig(destDir, { ticketSystem, jiraConfig, baseBranch, forgeType }) {
  ...
  const content = [
    ...
    '  transitions:',
    `    on_pr: ${yamlString(jiraConfig.done_transition_id)}    # used by /nova-wrap → "Code Review"`,
    `    done: ${yamlString(jiraConfig.done_transition_id)}    # legacy; Jira moves to Done on merge via forge integration`,
  ].join('\n') + '\n';
