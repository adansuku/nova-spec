...
async function sync(destDir = process.cwd()) {
  ...
  // Backfill context/ READMEs into already-installed repos. Pre-1.0.8 installs
  // don't have these. Idempotent: only writes missing files — the team's content
  // is never touched.
  if (fs.existsSync(path.join(destDir, 'context'))) {
    try {
      const { scaffoldContextReadmes } = require('./installer.js');
      scaffoldContextReadmes(destDir);
    } catch (_) {
      // installer.js requires inquirer at top level. If we're in a stripped
      // env (e.g. node_modules pruned), skip the backfill silently.
    }
  }
